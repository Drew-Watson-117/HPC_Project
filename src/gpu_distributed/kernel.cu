...
__global__ void lineOfSightKernel(int16_t *data, int *results, int width, int height, int offsetY_begin, int offsetY_end, int range) {
    ...
    if (centerX < width && centerY <= offsetY_end){
        ...
        // printf("count at (%i, %i): %i\n", centerX, centerY, count);	
        results[(centerY * width + centerX) - (offsetY_begin * width)] = count;
    }
}
...
void launchLineOfSightKernel_MPI(int16_t *d_data, int *d_results, int width, int height, int offsetY_begin, int offsetY_end, int range) {
    ...
    printf("grid x dim: %d, grid y dim: %d\n", dimGrid.x, dimGrid.y);
...
extern "C++" void cuda_bootstrapper(std::vector<int16_t> &data, std::vector<int> &counts, int16_t range, int width, int height, int my_rank, int comm_sz, int rows_per_rank, int row_overflow){
    ...
    const int PROBLEM_SIZE = 6000 * 6000; //36 MILLION
    int16_t *d_data;
    int *d_counts;
    ...
    if(my_rank != comm_sz - 1){
        ...
        printf("offset y begin %d, offset y end %d\n", offsetY_begin, offsetY_end);
    }
    ...
    if(my_rank != comm_sz - 1){
	    ...
	    printf("rows per rank: %d, width of row %d\n", rows_per_rank, width);
    ...
    float time_elapsed = PROBLEM_SIZE * 4 / milliseconds/1e6;
    printf("time elapsed : %f\n", milliseconds);
    printf("rank %i, Effective bandwidth (GB/s): %f\n", my_rank, time_elapsed);
