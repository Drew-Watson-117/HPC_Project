...
*/
void cpu_shared(int thread_count, int16_t range=100);

int main(int argc, char const* argv[]) { 
	if (argc != 2) {
		std::cout << "Must specify thread count as a command line argument" << std::endl;
		return 1;
	} 
    ...
    // test();
	// timing(100);
    cpu_shared(thread_count,10);

    return 0;
}

void cpu_shared(int thread_count, int16_t range) {
