...
void MPI_line_of_sight(std::vector<int16_t> &data, std::vector<int> &counts, int16_t range, int my_rank, int comm_sz, int data_split, int data_overflow);
...
int main(int argc, char* argv[]) { 
    ...
    int data_overflow = DATA_COUNT % comm_sz;
    int data_split = ((DATA_COUNT - data_overflow) / comm_sz);
    ...
    MPI_line_of_sight(data, local_counts, 10, my_rank, comm_sz, data_split, data_overflow);
    MPI_Barrier(comm);
    ...
    if(my_rank == 0){
        ...
        std::ofstream file;
        file.open("cpu_distributed_out.raw", std::ios::binary);
        if (file.is_open()) {
